...
export interface ICommandOptionsPost extends ICommandOptions {
    verbose?: boolean
}

export default async function post(src: string, dst: string, options?: ICommandOptionsPost) : Promise<{
  ...
  let commandOptions = setOptionDefaults<ICommandOptionsPost>(options || {});
   ...
   **************************/
  if (!isDirectory(dst)) {
    throw new Error('POST destination must be a directory.')
  }
  ...
  if (isDirectoryContents(src)) {
    ...
    if(options?.verbose) { 
        writeErrorString(BashlibErrorMessage.postFileTarget, undefined, options) 
    }
    // Here, we fall back on posting the direct container contents
  }
